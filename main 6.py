...
async def get_auth_key() -> None:
    global global_auth_key
    with open("auth_key.txt", "r") as local_auth_key:
        global_auth_key = local_auth_key.read()
...
async def get_public_codes() -> list[str]:
    async with aiohttp.ClientSession() as session:
        headers = {"Authorization": global_auth_key, "User-Agent": "LucidTrackerV1"}
...
async def get_shared_group_data(headers, json):
    headers['X-Authorization'] = headers['X-Authorization'].strip()
